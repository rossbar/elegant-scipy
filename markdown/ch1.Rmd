...
The so-called [central dogma of molecular biology](https://en.wikipedia.org/wiki/Central_dogma_of_molecular_biology) states that all the information needed to run a cell (or an organism, for that matter) is stored in a molecule called *deoxyribonucleic acid*, or DNA (see +@fig:central_dogma).
...
+@fig:dna illustrates the basic chemical structure of DNA.

![The chemical structure of DNA (image by Madeleine Price Ball,
used under the terms of the CC0 public domain license)](../images/2000px-DNA_chemical_structure.png){#fig:dna height=40% placement='H'}
...
![Central dogma of molecular biology](../figures/central_dogma.png){#fig:central_dogma width=60%}
...
![Gene expression](../figures/differential_gene_expression.png){#fig:gene_expression placement='H'}

The state-of-the-art technology to measure mRNA is RNA sequencing (RNAseq).
The RNA sequencing procedure is illustrated in +@fig:rna_seq
...
![RNA sequencing (RNAseq)](../figures/RNAseq.png){#fig:rna_seq width=60%}
...
Table: Example gene expression count data {#tbl:gene_expr align=crr}

+@tbl:gene_expr shows a minimal example of gene expression count data:
The data is a table of integers representing how many reads were observed for each gene in each cell type.
...
## NumPy N-Dimensional Arrays {#sec:ndarray}
...
This is what we use to represent the data in +@tbl:gene_expr:
...
Now you can see that the `shape` attribute generalizes `len` to account for the size of multiple dimensions of an array of data, as illustrated in +@fig:ndarray.

![Visualizing NumPy's ndarrays in one, two and three dimensions](../figures/NumPy_ndarrays_v2.png){#fig:ndarray}
...
### Why Use ndarrays Instead of Python Lists? {#sec:ary_vs_list}
...
```{python ipub={'text': {'format': {'escapechar' : '\\%'}}}}
print(r"%", end="")
...
```{python ipub={'code': {'label': 'code:vect_mult', 'asfloat': True}, 'text': {'format': {'escapechar' : '\\%'}}}}
print(r"%", end="")
...
For example, an array of 64-bit integers takes up exactly 64-bits per element, plus some very small overhead for array metadata, such as the `shape` attribute we discussed in +@sec:ndarray.
This is generally much less than would be given to objects in a Python list[^vdp].

[^vdp]: If you're interested in digging into how Python memory allocation works,
...
In section +@code:vect_mult we talked about the speed of operations on arrays.
...
TODO - Figure out internal referencing for [later in this chapter].
...
We're first going to use [pandas](https://pandas.pydata.org/) to read in the table of counts.
...
Although NumPy has some support for mixed data types (i.e. [structured arrays](https://numpy.org/devdocs/user/basics.rec.html)), it is not primarily designed for
...
TODO: Prettify table with `nb_setup` or `IPython.display.display`
```{python}
import bz2
import numpy as np
import pandas as pd
...
TODO: figure out how to reference a block quote via the \ref mechanism
...
>
> First, `%matplotlib inline` is a Jupyter notebook [magic command](http://ipython.org/ipython-doc/dev/interactive/tutorial.html#magics-explained),
...
> on styles, check out the [Matplotlib documentation on stylesheets](http://matplotlib.org/users/style_sheets.html).

Now back to plotting our counts distribution! 

```{python ipub={'figure': {'label': 'fig:kde', 'caption': 'Density plot of gene expression counts per individual using KDE smoothing'}}}
...
Figure !@fig:kde visualizes the distribution of counts as estimated by KDE.
...
```{python ipub={'figure': {'label': 'fig:ge_box', 'caption': 'Boxplot of gene expression counts per individual'}}}
...
```{python ipub={'figure': {'label': 'fig:ge_box', 'caption': 'Boxplot of gene expression counts per individual (log scale)'}}}
...
```{python ipub={'figure': {'label': 'fig:ge_box', 'caption': 'Boxplot of library-normalized gene expression counts per individual (log scale)'}}}
...
Much better!
Also notice how we used broadcasting twice there: once to divide all the gene
expression counts by the total for that column, and then again to multiply all
the values by 1 million.
...
```{python ipub={'figure': {'label': 'fig:ge_box', 'caption': 'Comparison of raw and library-normalized gene expression counts in three samples (log scale)'}}}
...
Remember that in RNAseq experiment, we fragment the transcripts, and sample reads from that pool of fragments, as +@fig:gene_len illustrates.
...
![Relationship between counts and gene length](../figures/gene_length_counts.png){#fig:gene_len}
...
TODO: Reference from code comment below to callout in text
```{python}
...
```{python ipub={'figure': {'label': 'fig:lgen_len', 'caption': 'Total counts in RNAseq experiment as a function of gene length'}}}
with plt.style.context('style/thinner.mplstyle'):
    binned_boxplot(x=log_gene_lengths, y=mean_log_counts);
```

We can see in +@fig:lgen_len that the longer a gene is, the higher its measured counts! As
...
First, let's calculate reads per kilobase, which would be given 
by $\frac{C}{L}$.

The RPMK formula asks for reads per *kilobase* instead of reads per base.
...
This leaves us with +@eq:rpkm to calculate reads per kilobase transcript per
million reads:

$$RPKM = \frac{10^9C}{LN}$$ {#eq:rpkm}
...
TODO: Again, reference in code comment to a callout
```{python}
...
Let's leverage this in our application of the RPKM formula (+@eq:rpkm) to the
entire counts array.
...
TODO: Hint callout about in-place operators?
```{python}
...
TODO: Callout about docstrings & numpydoc docstring formatting?
...
Let's see the RPKM normalization's effect in action. First, as a reminder, let's
reproduce +@fig:lgen_len which shows the distribution of mean log counts as a
function of gene length:

```{python ipub={'figure': {'caption':'Reproduction of +@fig:lgen_len'}}}
...
```{python ipub={'figure': {'label': 'fig:rpkm', 'caption': 'Average counts versus gene length after RPKM normalization'}}}
...
```{python ipub={'figure': {'label': 'fig:gene_comp_unnorm', 'caption': 'Comparing expression of two genes before RPKM normalization'}}}
...
```{python ipub={'figure': {'label': 'fig:gene_comp_rpkm', 'caption': 'Comparing expression of two genes after RPKM normalization'}}}
...
A direct comparison of +@fig:gene_comp_unnorm and +@fig:gene_comp_rpkm 
clearly illustrates the necessity of normalization: we can now directly compare
